...
%
% THIS VERSTION GOT RID OF THE INTEGRAL IN THE DENOMINATOR!!!!!!
%
% 
...
function [derivative, energy] = and_CFD_datagen_mass_derivStat_all_parameters_totVar2...
...
%INTEGRAL DENOMINATOR            scale_p_ymkj = 1./sum(p_ymkj,1);
%            scale_p_ymkj_mat = diag(scale_p_ymkj); %Multiplying diag will
%            cost O(N^3). Elementwise will cost O(N^2).  
%INTEGRAL DENOMINATOR        scale_p_ymkj_mat = repmat(scale_p_ymkj,[num_particles,1]) ;
    %tilde_pym in the old version
%INTEGRAL DENOMINATOR         p_ymkj = p_ymkj .* scale_p_ymkj_mat;
            tilde_p_ymk = mean(p_ymkj,2);
            dEP_kr = 1/N *p_ymkj * deriv_loglike' ;
            
            derivative_alltime(:,snaptime_now) = sign(tilde_p_ymk - ...
                compress_snap_wgts(:,snaptime_now))' * (dEP_kr);    
            energy(snaptime_now) = sum(abs(tilde_p_ymk -compress_snap_wgts(:,snaptime_now)));
